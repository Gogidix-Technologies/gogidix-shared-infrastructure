package com.gogidix.shared.ecommerce.admin.export;

import java.io.OutputStream;
import java.util.List;
import java.util.Map;

/**
 * Interface for handling custom export templates.
 *
 * @param <T> The type of data to export
 */
public interface ExportTemplateHandler<T> {

    /**
     * Export data using a custom template.
     *
     * @param data The data to export
     * @param outputStream The output stream to write to
     * @param options Additional export options
     */
    void export(List<T> data, OutputStream outputStream, Map<String, Object> options) throws ExportException;
    
    /**
     * Get the name of the template this handler provides.
     *
     * @return The template name
     */
    String getTemplateName();
    
    /**
     * Get the output format of this template.
     *
     * @return The export format
     */
    ExportFormat getOutputFormat();
    
    /**
     * Check if this handler supports the given data type.
     *
     * @param dataType The data type to check
     * @return true if supported, false otherwise
     */
    default boolean supports(Class<?> dataType) {
        return true; // Default implementation supports all types
    }
}
